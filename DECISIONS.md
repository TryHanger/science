...
6. **Признаки.** ResNet-50 (`ResNet50_Weights.DEFAULT`, Resize 224×224, ImageNet-нормализация) **без изменений** — кэш признаков v3/v4 переиспользуется (resume извлечения). Индекс изображений строится одним проходом по каталогу; отсутствующие изображения пропускаются и логируются; обучение прерывается, если без признаков > 1% вопросов (`max_missing_feature_frac: 0.01`). Признаки для обучения загружаются в RAM (`preload_features: true`, ≈1 GB) — иначе чтение gzip-H5 по одному примеру доминирует во времени эпохи.
