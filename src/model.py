...
class VQAModel(nn.Module):
    ...
    - ????????? ?????:  Word Tokens -> Embedding -> LSTM (pack_padded_sequence) -> Linear -> [common_proj_dim] -> Tanh
    - ??????? (Fusion):
        * 'mul'    ? ???????????? ???????????? ??????? (Element-wise multiplication)
        * 'concat' ? ???????????? ????????
    - ?????????????:   Dropout(0.5) -> Linear -> ReLU -> Dropout(0.5) -> Linear -> Logits [num_answers]
    """
    def __init__(
        self,
        vocab_size: int,
