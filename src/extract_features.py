...
def find_image_path(img_dir: Path, split_name: str, image_id: int) -> Path:
    """
    Find the path to a COCO image by image_id.
    If exact filename is not found, search with pattern matching.
    """
    candidates = [
    ...
    raise FileNotFoundError(
        f"\n[Data Error] Image for image_id={image_id} not found in directory: {img_dir}\n"
        f"Check that MS COCO 2014 image dataset is downloaded to this path."
    )
...
class ResNet50FeatureExtractor(nn.Module):
    ...
    Removes the final fc layer and keeps avgpool (feature dimension 2048).
    """
    def __init__(self):
        super().__init__()
...
def extract_features_for_split(
    ...
    print(f"\n=======================================================")
    print(f"Extracting features for split: {split_name.upper()}")
    print(f"Questions path:  {questions_json_path}")
    print(f"Images dir:      {img_dir_path}")
    print(f"Output H5:       {output_h5_path}")
    print(f"Device:          {device}")
    print(f"=======================================================")

    # Check data availability (on Kaggle or locally)
    if not os.path.exists(questions_json_path):
        raise FileNotFoundError(
            f"\n[Data Error] Questions file not found: {questions_json_path}.\n"
            f"For Kaggle: add the VQA v2 dataset via 'Add Input' and verify paths in configs/kaggle.yaml.\n"
    ...
    if os.path.exists(output_h5_path):
        ...
        print(f"[2/3] Found existing H5 file. Already extracted images: {len(existing_ids):,}")
